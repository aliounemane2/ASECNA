...
if ((isset($_SESSION['id_user'])) && (!empty($_SESSION['id_user']))) :
    if ($_SESSION['role'] == 1) {
        ...
        $n = "SELECT *, COUNT(*) AS nb_total, date_format(ANNONCE_DATE_DEB , '%d/%m/%Y') as date_debut, date_format(ANNONCE_DATE_FIN, '%d/%m/%Y') as date_fin,date_format(ANNONCE_DELAI_AGE, '%d/%m/%Y') as delai
        FROM annonce a WHERE a.ANNONCE_ID NOT IN ( SELECT p.FK_annonce_id FROM postulation p WHERE p.FK_UTIL_ID =".$_SESSION['id_user'].")
        AND ANNONCE_DATE_FIN >= CURRENT_DATE";
        $nb_total = mysql_query($n);
        $nb_total = mysql_fetch_array($nb_total);
        $nb_total = $nb_total['nb_total'];
    }
...
$sql = "SELECT *, date_format(ANNONCE_DATE_DEB , '%d/%m/%Y') as date_debut, date_format(ANNONCE_DATE_FIN, '%d/%m/%Y') as date_fin, date_format(ANNONCE_DELAI_AGE, '%d/%m/%Y') as delai
		FROM annonce
		WHERE ANNONCE_DATE_FIN >= CURRENT_DATE
   		order by ANNONCE_DATE_CREATION DESC LIMIT $limit_start, $pagination";
...
?>
		...
		<?php if ((isset($_SESSION['login'])) && (!empty($_SESSION['login'])) && $_SESSION['role'] == 1) : ?>
				<a href="index.php?url=listeavp&id_annonce=<?php echo $data['ANNONCE_ID']?>" class="navhaut"><b>Voir les avps </b></a>
		<?php else : ?>
...
<!--  source pour faire la rechere  -->
 ...
 if(isset($_POST["rechercher"]))
 {
     ...
     $sql = "SELECT *, date_format(ANNONCE_DATE_DEB , '%d/%m/%Y') as date_debut, date_format(ANNONCE_DATE_FIN, '%d/%m/%Y') as date_fin, date_format(ANNONCE_DELAI_AGE, '%d/%m/%Y') as delai
		FROM annonce
		WHERE ANNONCE_DATE_FIN >= CURRENT_DATE  AND SECTEUR='$secteur'
   		order by ANNONCE_DATE_CREATION DESC LIMIT $limit_start, $pagination";
