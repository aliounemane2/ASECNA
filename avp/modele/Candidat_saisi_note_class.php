...
function __construct()
  {
	...
	$this->table_candidat_saisi_note='candidat_saisi_note';
	$this->champ_id='id';
	...
	$this->champ_observation='observation';
...
function GetCh_observation(){return $this->champ_observation;}
...
// création de la fonction Lister candidat_saisi_note
  ...
  function lister_candidat_saisi_note_by_annonce($cle="",$trie="ASC",$nbre_enreg="")
  {
	$le_champ_cle=$this->GetCh_FK_annonce_id();
  ...
  function getObservationById_anonce($annonce_id){
	  
	    $req = sprintf("SELECT observation FROM ".$this->Get_table_candidat_saisi_note()." WHERE  ".$this->GetCh_FK_annonce_id()."=%s AND "
		.$this->GetCh_observation(). "!= '' ",
		...
		if($resultat!=NULL){
		return $resultat[0][$this->GetCh_observation()]; 
		}else{
			return NULL;
		}
...
function ajouter_enreg_candidat_saisi_note($param,$Cnn)
  {
	...
	$this->GetSQLValueString($param['observation'], "text"),
